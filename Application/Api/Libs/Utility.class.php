...
class Utility
{
     ...
     *  
     */
    public static function encodeHtmlName($id)
    {
    	$first = dechex(($id*$id)%15);
    	$last = dechex(($id+1)%(hexdec($first)+1));
    	return $first.dechex(12345+$id).$last;
    }
    
    /**
     * 解码生成后的静态页面的文件名
     * 
     * @param string $str 静态页面名
     * 
     */
    public static function decodeHtmlName($str) {
    	$inte = substr($str, 1, -1);
    	$id = hexdec($inte);
    	return $id-12345;
    }
     ...
     * 
     */
    public static function rermdir($dir)
    {
    	if (is_dir($dir)) {
    		$objects = scandir($dir);
    		...
    		reset($objects);
    		if(substr($dir, 0, -strlen(strrchr($dir, '/'))) != $_SERVER["DOCUMENT_ROOT" ]) {//只删除此目录里的所有东西，保留此目录
    			rmdir($dir);
    		}
    	}
    }
